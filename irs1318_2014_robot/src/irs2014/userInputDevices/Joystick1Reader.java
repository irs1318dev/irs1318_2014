...
public class Joystick1Reader extends RobotComponentBase {
	...
	public void teleopPeriodic() {
		...
		joystickY = -JoystickFilter.applyLinearDeadBand(joystick.getY(), 0.1);	//changed to positive for competition bot 

		ReferenceData.getInstance().getUserInputData().setJoystickY(joystickX);	//flipped x and y for competition bot 
...
//		ReferenceData.getInstance().getUserInputData().setGoForward(getGoForward());
	...
	private boolean getShoot3Pistons() {
		return joystick.getRawButton(ButtonRef.SHOOT_3_PISTONS);
	}
